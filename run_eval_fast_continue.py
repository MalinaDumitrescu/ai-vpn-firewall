#!/usr/bin/env python
"""
FAST CONTINUATION: Parts 3-7 of comprehensive evaluation.
Optimized: 3 seeds, 150 trees, XGB-only for LODO to finish in reasonable time.
"""
...
def train_ens(X_tr, y_tr, X_val, y_val, n_est=150):
    """Train ensemble: XGB + LGB + CatBoost."""
    models = {}
    try:
...
def train_xgb_only(X_tr, y_tr, X_val, y_val, n_est=150, sample_weight=None):
    """Fast: XGB only for speed in LODO."""
    import xgboost as xgb
    m = xgb.XGBClassifier(n_estimators=n_est, max_depth=5, learning_rate=0.05,
        subsample=0.8, colsample_bytree=0.8,
        scale_pos_weight=max(1.0,(y_tr==0).sum()/max((y_tr==1).sum(),1)),
    ...
    m.fit(X_tr, y_tr, eval_set=[(X_val, y_val)], verbose=False, **kw)
    return m
...
def ens_pred(models, X):
    if isinstance(models, dict):
        ...
        return models.predict_proba(X)[:,1]
...
def main():
    ...
    N_EST = 150  # reduced for speed

    # ================================================================
    # PART 3: LODO (XGB-only for speed)
    # ================================================================
    lodo_path = OUT / "clean_lodo_results.csv"
    if lodo_path.exists():
        print(f"\nLODO results already exist, loading...")
        lodo_df = pd.read_csv(lodo_path)
        print(f"  Loaded {len(lodo_df)} rows")
    else:
        print("\n" + "="*70)
        print("PART 3: REPEATED LODO (XGB-only for speed)")
        print("="*70)
        ...
        for fam_name, fam_cols in FAMILIES.items():
            ...
            if missing:
                ...
                continue
            print(f"\n  --- LODO: {fam_name} ({len(fam_cols)} features) ---")
            for held in datasets:
                ...
                for seed in range(42, 42+N_SEEDS):
                    ...
                    X_tr, y_tr = src_tr[fam_cols].values, src_tr.label.values
                    X_va, y_va = src_va[fam_cols].values, src_va.label.values
                    X_te, y_te = tgt[fam_cols].values, tgt.label.values
                    if len(np.unique(y_tr)) < 2:
                        continue
                    m = train_xgb_only(X_tr, y_tr, X_va, y_va, n_est=N_EST)
                    ens_va = m.predict_proba(X_va)[:,1]
                    ens_te = m.predict_proba(X_te)[:,1]
                    ...
                    lodo_rows.append({
                        "held_out": held, "seed": seed, "family": fam_name,
                        "flow_auc": safe_auc(y_te, ens_te),
                        "flow_ap": safe_ap(y_te, ens_te),
                        "sess_auc_wt5": sess_auc,
                        "recall": cm["recall"], "fpr": cm["fpr"], "threshold": thr,
                ...
                if sub:
                    ...
                    print(f"    held={held}: AUC={np.mean(aucs):.4f}+/-{np.std(aucs):.4f}, "
                          f"recall={np.mean(recs):.4f} ({elapsed:.0f}s)")
    ...
    # ================================================================
    # PART 4: DOMAIN-BALANCED TRAINING
    # ================================================================
    print("\n" + "="*70)
    print("PART 4: DOMAIN-BALANCED WEIGHTING")
    print("="*70)

    best_fam = None; best_wr = -1
    for fam in FAMILIES:
    ...
    print(f"  Domain weights: {dict(zip(ds_counts.index, [round(max_ds/v,2) for v in ds_counts.values]))}")
    ...
    try:
        ...
        m_dw = train_xgb_only(X_tr, y_tr, X_va, y_va, N_EST, sample_weight=sample_weights)
        ...
        for held in datasets:
            ...
            m2 = train_xgb_only(src_tr[fam_cols].values, src_tr.label.values,
                                src_va[fam_cols].values, src_va.label.values,
                                N_EST, sample_weight=sw)
    ...
    ens_te_scores = ens_pred(models, te[fam_cols].values)
    y_te = te.label.values
    te_pred = te[["capture_id","dataset","label"]].copy()
    te_pred["ensemble_score"] = ens_te_scores
    sess = session_agg(te_pred)
    y_sess = sess.label.values
    print(f"  Sessions: {len(sess)} ({sum(y_sess)} VPN, {len(y_sess)-sum(y_sess)} nonVPN)")
    ...
    deploy = {}
    for name, sub_df in [("strict",strict),("balanced",balanced),("flag",flag)]:
        if len(sub_df)>0:
            b = sub_df.iloc[0]
            deploy[name] = {k: (float(v) if isinstance(v, (np.floating, np.integer)) else v) for k,v in b.to_dict().items()}
            print(f"  {name.upper()}: {b['agg']}@{b['thr']}: recall={b['recall']:.4f}, "
                  f"fpr={b['fpr']:.4f}, worst_fpr={b['worst_fpr']:.4f}, worst_rec={b['worst_recall']:.4f}")
        else:
            print(f"  {name.upper()}: no valid policy found")
            deploy[name] = None
    ...
    for fam in FAMILIES:
        ...
        if "sess_mean_score_auc" in sub.columns:
            cand["sess_auc"] = f"{sub.sess_mean_score_auc.mean():.4f}"
        for ds in datasets:
            k = f"{ds}_recall"
            if k in sub.columns: cand[f"{ds}_recall"] = f"{sub[k].mean():.4f}+/-{sub[k].std():.4f}"
            k = f"{ds}_fpr"
            if k in sub.columns: cand[f"{ds}_fpr"] = f"{sub[k].mean():.4f}+/-{sub[k].std():.4f}"
        if len(lsub)>0:
    ...
    usb_fragile = bool(usb_n.mean() < 100) if len(usb_n)>0 else True
