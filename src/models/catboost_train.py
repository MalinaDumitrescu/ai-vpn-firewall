...
try:
    ...
    raise ImportError(
        "catboost is not installed. Add it to pyproject.toml / pip install catboost."
    ) from e
...
def train_catboost(
    *,
    ...
    metrics_path = (
        paths.repo_root / str(out_cfg.get("metrics_path", "artifacts/catboost/metrics.json"))
    ).resolve()
    preds_path = (
        paths.repo_root / str(out_cfg.get("preds_path", "artifacts/catboost/preds.parquet"))
    ).resolve()
    ...
    metrics: Dict[str, Any] = {
        ...
        "catboost_yaml": str(catboost_yaml.resolve()),
        "seed": seed,
