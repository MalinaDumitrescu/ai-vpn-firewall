...
try:
    ...
    raise ImportError(
        "lightgbm is not installed. Add it to pyproject.toml / pip install lightgbm."
    ) from e
...
def train_lightgbm(
    *,
    ...
    lgbm_params = dict(cfg.get("lgbm_params") or {})
    lgbm_params["seed"] = seed
    lgbm_params.setdefault("objective", "binary")
    lgbm_params.setdefault("metric", ["binary_logloss", "auc"])
    lgbm_params.setdefault("verbosity", -1)

    out_cfg = cfg.get("outputs") or {}
    model_path = (
        paths.repo_root / str(out_cfg.get("model_path", "artifacts/lgbm/model.txt"))
    ).resolve()
    metrics_path = (
        paths.repo_root / str(out_cfg.get("metrics_path", "artifacts/lgbm/metrics.json"))
    ).resolve()
    preds_path = (
        paths.repo_root / str(out_cfg.get("preds_path", "artifacts/lgbm/preds.parquet"))
    ).resolve()
    ...
    # Predict using best iteration (default behavior in lgb.train with early stopping)
    p_train = booster.predict(X_train)
    p_val = booster.predict(X_val)
    p_test = booster.predict(X_test)
    ...
    metrics: Dict[str, Any] = {
        ...
        "lgbm_yaml": str(lgbm_yaml.resolve()),
        "seed": seed,
    ...
    preds.loc[train_df.index, "p_lgbm"] = p_train
